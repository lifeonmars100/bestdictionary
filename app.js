document.addEventListener('DOMContentLoaded', () => {
    ...
    // ----- 상수 -----
    // 한글 검색 기능 추가를 위해 데이터베이스 버전을 3으로 업그레이드합니다.
    ...
    function openDB() {
        return new Promise((resolve, reject) => {
            ...
            // 데이터베이스 생성 및 업그레이드를 모두 처리하는 완전한 로직
            request.onupgradeneeded = e => {
                const db = e.target.result;
                let store;
                // 저장소(store)가 없으면 새로 생성 (최초 실행 시)
                if (!db.objectStoreNames.contains(STORE_NAME)) {
                    store = db.createObjectStore(STORE_NAME, { keyPath: "id" });
                } 
                // 저장소가 이미 있으면 기존 저장소 가져오기 (버전 업그레이드 시)
                else {
                    store = e.target.transaction.objectStore(STORE_NAME);
                }

                // 각 인덱스가 없을 때만 생성하여 안전하게 업그레이드
                ...
                // 한글 검색을 위한 검색 태그용 인덱스 추가
                if (!store.indexNames.contains('searchTags_idx')) {
                    // multiEntry: true 옵션은 배열의 각 항목을 개별적으로 인덱싱합니다.
    ...
    // JSON 데이터를 가져올 때, 검색용 태그(searchTags)를 자동으로 생성합니다.
    function importDataToDB(wordsData) {
        return new Promise((resolve, reject) => {
            ...
            wordsData.forEach(word => {
                if (word.korean) {
                    // 쉼표(,) 또는 슬래시(/)로 뜻을 분리하고, 각 뜻의 공백을 제거하여 태그 배열 생성
    ...
    // 모든 인덱스를 활용하는 새로운 통합 검색 함수
    function searchWords(term) {
        return new Promise((resolve, reject) => {
            const searchTerm = term.trim();
            if (!db || searchTerm.length === 0) {
                resolve([]);
                return;
            }
            ...
            // 3가지 방식의 검색(영어, 한글시작, 한글포함)을 동시에 진행합니다.
            const searches = [
            ...
            // 모든 검색이 완료되면 결과를 합쳐 중복을 제거한 뒤 반환합니다.
