document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★★★★★ [문제점 1 최종 해결] 지능적인 단어/품사 분리 함수 ★★★★★
    const KNOWN_PARTS_OF_SPEECH = new Set(['n', 'v', 'vi', 'vt', 'a', 'ad', 'adj', 'pron', 'prep', 'conj', 'x']);
    function parseEnglishWord(rawEng) {
        // 가장 긴 품사부터(vt, adj) 순서대로 확인
        for (let len = 3; len > 0; len--) {
            if (rawEng.length <= len) continue; // 단어보다 긴 품사는 체크하지 않음
            
            const tail = rawEng.slice(-len);
            const head = rawEng.slice(0, -len);
            
            // 미리 정의된 품사 목록에 있는지 확인
            if (KNOWN_PARTS_OF_SPEECH.has(tail)) {
                // 'head'에 알파벳 외 다른 문자가 섞여있으면 파싱하지 않음 (예: 'beam w')
                if (/^[a-zA-Z]+$/.test(head)) {
                    return `${head} <span style="font-size: 0.8em; color: #ffffff99;">(${tail})</span>`;
                }
            }
        }
        // 분리할 수 없으면, 공백을 품사 구분자로 간주하여 처리 (예: 'beam w')
        const parts = rawEng.split(' ');
        if (parts.length > 1) {
            return `${parts[0]} <span style="font-size: 0.8em; color: #ffffff99;">(${parts.slice(1).join(' ')})</span>`;
        }
        
        return rawEng; // 어떤 규칙에도 해당하지 않으면 원본 반환
    }
    ...
    speakButton.addEventListener('click', () => {
        if (currentWord && 'speechSynthesis' in window) {
            // ★★★★★ 품사를 제거한 순수 단어만 읽도록 수정 ★★★★★
            const pureWord = currentWord.english.split(' ')[0].replace(/(n|v|vi|vt|a|ad|adj|pron|prep|conj|x)$/, '');
