document.addEventListener('DOMContentLoaded', () => {
    ...
    // ★★★★★ [문제점 1 해결] 단어/품사 분리 함수 ★★★★★
    function parseEnglishWord(rawEng) {
        // 끝에서부터 품사로 의심되는 부분을 찾습니다. (최대 3글자)
        for (let i = Math.min(rawEng.length, 3); i > 0; i--) {
            const tail = rawEng.slice(-i);
            const head = rawEng.slice(0, -i);
            // 품사가 모두 알파벳 소문자로만 이루어져 있다고 가정
            if (tail === tail.toLowerCase() && head.length > 1) {
                return `${head} <span style="font-size: 0.8em; color: #ffffff99;">(${tail})</span>`;
            }
        }
        return rawEng; // 분리할 수 없으면 원본 반환
    }
    ...
    function openDB() {
        return new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = e => {
                ...
                if (!store.indexNames.contains('searchTags_idx')) {
                    store.createIndex("searchTags_idx", "searchTags", { unique: false, multiEntry: true });
                }
            };
        });
    }
    ...
    function importDataToDB(wordsData) {
        return new Promise((resolve, reject) => {
            ...
            wordsData.forEach(word => {
                if (word.korean) {
                    word.searchTags = word.korean.split(/[,/]/).map(tag => tag.trim()).filter(tag => tag.length > 0);
                } else {
                    word.searchTags = [];
                }
                store.add(word);
    ...
    function displayWords(words) {
        ...
        words.forEach(word => {
            ...
            // parseEnglishWord 함수로 가공된 HTML을 innerHTML로 설정
    ...
    // ★★★★★ [문제점 1 해결] 플래시카드 표시에 파싱 함수 적용 ★★★★★
    function showDetailView(word) {
        currentWord = word;
        // 여기도 parseEnglishWord 함수 적용
    ...
    speakButton.addEventListener('click', () => { if (currentWord && 'speechSynthesis' in window) { const utterance = new SpeechSynthesisUtterance(currentWord.english.replace(/n$|v$|vi$|vt$|a$|ad$|adj$/, '')); window.speechSynthesis.speak(utterance); } });
